<?php
/**
 * ExportCertImagesCommandInvalidDestTest
 *
 * @see ExportCertImagesCommand
 * @author KURIHARA Hikonobu (Hymena & Co.)
 */

class ExportCertImagesCommandInvalidDestTest extends CDbTestCase
{
    /**
     * @var array Fixtures
     */
    ...
    public function testRun_1()
    {
        ...
        Yii::app()->params['file_transfer_cert_path'] = 'not_existing_dir/';

        $fids = array('testfile0', 'testfile2', 'testfile4', 'testfile6', 'testfile8', 'testfile1', 'testfile3', 'testfile5', 'testfile7', 'testfile9',);

        foreach ($fids as $fid) {
            $file_path_from = Yii::app()->params['file_transfer_cert_path'] . $this->file_transfer($fid)->filename;
